...
VALUES ('$pemilik_lama', '$pemilik_baru', '$alamat', '$no_kendaraan', '$no_uji', '$jenis', '$model', '$kategori', '$merk', '$type',  '$tahun', '$cc', '$bahan_bakar', '$angkut_orang',  '$angkut_barang', '$masa_berlaku', '$status_penggunaan','$tgl_mutasi','$jenis_mutasi', '$lokasi_mutasi', '0','$sekarang')"; //echo $strsql;

if (empty($pemilik_baru))
{
echo "<script>alert('Nama Pemilik Tidak Boleh Kosong'); document.location.href='index.php?act=add&do=data.mutasi_keluar&pemilik_lama=$pemilik_lama&pemilik_baru=$pemilik_baru&alamat=$alamat&no_kendaraan=$no_kendaraan&no_uji=$no_uji&jenis=$jenis&model=$model&kategori=$kategori&merk=$merk&type=$type&tahun=$tahun&cc=$cc&angkut_orang=$angkut_orang&angkut_barang=$angkut_barang&masa_berlaku=$masa_berlaku&status_penggunaan=$status_penggunaan&tgl_mutasi=$tgl_mutasi';</script>\n";
exit();
}
...
{
mysql_query($strsql);
header("location:index.php?do=data.mutasi_keluar");
}
?>