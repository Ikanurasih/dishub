...
<table width="1290" border="0" cellpadding="2" cellspacing="2">
  ...
  <form method="post" action="index.php?do=data.simpang_apill">
    <td >Pencarian & Cetak: &nbsp;<select name="kategori" class="inputbox" onChange=" submit(); ">
      <option value="b.nama_simpang" <?php if($kategori == "b.nama_simpang") echo "selected"; ?>>Nama Simpang</option>
      <option value="a.nama_kaki" <?php if($kategori == "a.nama_kaki") echo "selected"; ?>>Nama Kaki</option>
      <option value="a.rambu" <?php if($kategori == "a.rambu") echo "selected"; ?>>Rambu Apill</option>
      <option value="a.stopline" <?php if($kategori == "a.stopline") echo "selected"; ?>>Marka Stopline</option>
      <option value="a.ltor" <?php if($kategori == "a.ltor") echo "selected"; ?>>LTOR/ Belok Kiri Langsung</option>
    </select></td>
    </form>
    <form method="post" action="index.php?do=data.simpang_apill">
    <input type="hidden" name="kategori" value="<?php echo $kategori; ?>">
    <td>
    <?php if($kategori == "a.rambu" || $kategori == "a.stopline" || $kategori == "a.ltor"){ ?>
    	<?php if($kategori == "a.rambu" || $kategori == "a.stopline"){ ?>
        	<input type="radio" name="key" value="1" onClick=" submit(); " <?php if($key == "1" ) echo "checked"; ?>> Ada &nbsp; <input type="radio" name="key" value="2" onClick=" submit(); " <?php if($key == "2") echo "checked"; ?>> Tidak Ada
        <?php }else{ ?> 
        	<input type="radio" name="key" value="1" onClick=" submit(); " <?php if($key == "1") echo "checked"; ?>> Iya &nbsp; <input type="radio" name="key" value="2" onClick=" submit(); " <?php if($key == "2") echo "checked"; ?>> Tidak
        <?php } ?>
    <?php }else{ ?>   
	...
	<td width="458" align="right"><a href="excel_simpang_apill.php?kategori=<?php echo $kategori; ?>&key=<?php echo $key; ?>" target="_blank"><img src="images/excel.jpg" width="68" height="23" border="0"></a>&nbsp;&nbsp;<a href="index.php?do=data.simpang_apill&act=cetak&kategori=<?php echo $kategori; ?>&key=<?php echo $key; ?>"><img src="images/cetak.gif" title="Klik disini untuk Cetak Laporan" border="0"></a></td>
...
<table border="1" cellspacing="0" cellpadding="1" width="1290">
  ...
  if (!empty($key))
  {  
	$strsql="select a.*,b.nama_simpang, b.jumlah_fase from lal_simpang_apill a, lal_simpang b where a.id_simpang=b.id and $kategori like '%$key%' order by id desc"; 
  }
  else
